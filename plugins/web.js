...
/*
var LANG = { RES: '' }
if (Config.LANG == 'AR') LANG.RES == '*Komut Bulunamadı!*\n*Benzer Komut:*'
...
Asena.addCommand({pattern: 'سرعة النت ?(.*)', fromMe: true, desc: Lang.SPEEDTEST_DESC, usage: 'speedtest user // speedtest server'}, (async (message, match) => {
...
Asena.addCommand({pattern: 'بنج$', fromMe: true, deleteCommand: false, desc: Lang.PING_DESC}, (async (message, match) => {
  var start = new Date().getTime();
...
if (Config.WORKTYPE == 'private') {
    ...
    Asena.addCommand({pattern: 'احسب ?(.*)', fromMe: true, desc: Lang.CALC }, (async (message, match) => {
    ...
    Asena.addCommand({pattern: 'احسب ?(.*)', fromMe: false, desc: Lang.CALC }, (async (message, match) => {
        if (match[1].length < 4) { return await message.client.sendMessage(message.jid,Lang.VALİD, MessageType.text) }
        if (match[1].includes('+')) { var split = match[1].split('+'), sonsayi = split[1], ilksayi = split[0]
            var result = -(-ilksayi - sonsayi)
            try { await message.client.sendMessage(message.jid,Lang.SUC + result, MessageType.text) }
