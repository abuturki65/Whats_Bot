...
var cmd = ''
var cmd_desc = ''
if (Config.LANG == 'AR') description = '- يعـرض كل أوامـر تحـويـلات النصـوص الـى ملصقـات ⌨---🎇', cmd = '*الامـر:* ', cmd_desc = '*الوصـف:* '
...
Asena.addCommand({ pattern: 'allttp$', fromMe: wk, desc: description }, (async (message, match) => {
  ...
  var t3 = Config.LANG == 'AR' || Config.LANG == 'AZ' ? "يحول النـص إلى ملصـق ذو طابـع مائـي...🏞" : "Converts text to water-themed sticker."
  var t4 = Config.LANG == 'AR' || Config.LANG == 'AZ' ? "يحول النـص إلى ملصـق الكتابـة باليـد...🙌" : "Converts text to hand writing sticker."
  var t5 = Config.LANG == 'AR' || Config.LANG == 'AZ' ? "يحول النـص إلى ملصـق ذي طابع البـلاك بيرد...👽" : "Converts text to blackbird-themed sticker."
  var t6 = Config.LANG == 'AR' || Config.LANG == 'AZ' ? "Yazıyı sakız temalı sticker'e çevirir." : "Converts text to gum-themed sticker."
  var t7 = Config.LANG == 'AR' || Config.LANG == 'AZ' ? "Yazıyı şirinler temalı sticker'e çevirir." : "Converts text to smurfs-themed sticker."
  var t8 = Config.LANG == 'AR' || Config.LANG == 'AZ' ? "Yazıyı elektrik temalı sticker'e çevirir." : "Converts text to electric-themed sticker."
  var t9 = Config.LANG == 'AR' || Config.LANG == 'AZ' ? "Yazıyı vurgulayan animasyonlu sticker'e çevirir." : "Converts text to highlighted animated sticker."
  var t10 = Config.LANG == 'AR' || Config.LANG == 'AZ' ? "Yazıyı geçiş animasyonlu sticker'e çevirir." : "Converts text to transition animated sticker."
...
Asena.addCommand({ pattern: 'wttp ?(.*)', fromMe: wk, dontAddCommandList: true }, (async (message, match) => {
  if (message.reply_message) {
    ...
    await download(ttinullimage.image, '/root/Whats_Bot/wttp.png', async() => { 
      ffmpeg('/root/Whats_Bot/wttp.png').videoFilters('chromakey=white').save('af.png').on('end', async () => {
        ffmpeg('/root/Whats_Bot/af.png').outputOptions(["-y", "-vcodec libwebp"]).videoFilters('scale=2000:2000:flags=lanczos:force_original_aspect_ratio=decrease,format=rgba,pad=2000:2000:(ow-iw)/2:(oh-ih)/2:color=#00000000,setsar=1').save('st.webp').on('end', async () => {
...
Asena.addCommand({ pattern: 'http ?(.*)', fromMe: wk, dontAddCommandList: true }, (async (message, match) => {
  if (message.reply_message) {
    ...
    await download(ttinullimage.image, '/root/Whats_Bot/http.png', async() => { 
      ffmpeg('/root/Whats_Bot/http.png').videoFilters('chromakey=white').save('af.png').on('end', async () => {
        ffmpeg('/root/Whats_Bot/af.png').outputOptions(["-y", "-vcodec libwebp"]).videoFilters('scale=2000:2000:flags=lanczos:force_original_aspect_ratio=decrease,format=rgba,pad=2000:2000:(ow-iw)/2:(oh-ih)/2:color=#00000000,setsar=1').save('st.webp').on('end', async () => {
          await message.sendMessage(fs.readFileSync('st.webp'), MessageType.sticker);
        })
      })
...
Asena.addCommand({ pattern: 'bttp ?(.*)', fromMe: wk, dontAddCommandList: true }, (async (message, match) => {
  if (message.reply_message) {
    ...
    await download(ttinullimage.image, '/root/Whats_Bot/bttp.png', async() => { 
      ffmpeg('/root/Whats_Bot/bttp.png').videoFilters('chromakey=white').save('af.png').on('end', async () => {
        ffmpeg('/root/Whats_Bot/af.png').outputOptions(["-y", "-vcodec libwebp"]).videoFilters('scale=2000:2000:flags=lanczos:force_original_aspect_ratio=decrease,format=rgba,pad=2000:2000:(ow-iw)/2:(oh-ih)/2:color=#00000000,setsar=1').save('st.webp').on('end', async () => {
          await message.sendMessage(fs.readFileSync('st.webp'), MessageType.sticker);
        })
      })
...
Asena.addCommand({ pattern: 'gttp ?(.*)', fromMe: wk, dontAddCommandList: true }, (async (message, match) => {
  if (message.reply_message) {
    ...
    await download(ttinullimage.image, '/root/Whats_Bot/gttp.png', async() => { 
      ffmpeg('/root/Whats_Bot/gttp.png').videoFilters('chromakey=white').save('af.png').on('end', async () => {
        ffmpeg('/root/Whats_Bot/af.png').outputOptions(["-y", "-vcodec libwebp"]).videoFilters('scale=2000:2000:flags=lanczos:force_original_aspect_ratio=decrease,format=rgba,pad=2000:2000:(ow-iw)/2:(oh-ih)/2:color=#00000000,setsar=1').save('st.webp').on('end', async () => {
          await message.sendMessage(fs.readFileSync('st.webp'), MessageType.sticker);
        })
      })
...
Asena.addCommand({ pattern: 'sttp ?(.*)', fromMe: wk, dontAddCommandList: true }, (async (message, match) => {
  if (message.reply_message) {
    ...
    await download(ttinullimage.image, '/root/Whats_Bot/sttp.png', async() => { 
      ffmpeg('/root/Whats_Bot/sttp.png').videoFilters('chromakey=white').save('af.png').on('end', async () => {
        ffmpeg('/root/Whats_Bot/af.png').outputOptions(["-y", "-vcodec libwebp"]).videoFilters('scale=2000:2000:flags=lanczos:force_original_aspect_ratio=decrease,format=rgba,pad=2000:2000:(ow-iw)/2:(oh-ih)/2:color=#00000000,setsar=1').save('st.webp').on('end', async () => {
          await message.sendMessage(fs.readFileSync('st.webp'), MessageType.sticker);
        })
      })
...
Asena.addCommand({ pattern: 'ettp ?(.*)', fromMe: wk, dontAddCommandList: true }, (async (message, match) => {
  if (message.reply_message) {
    ...
    await download(ttinullimage.image, '/root/Whats_Bot/ettp.png', async() => { 
      ffmpeg('/root/Whats_Bot/ettp.png').videoFilters('chromakey=#FFFFFF:similarity=0.01').save('af.png').on('end', async () => {
        ffmpeg('/root/Whats_Bot/af.png').outputOptions(["-y", "-vcodec libwebp"]).videoFilters('scale=2000:2000:flags=lanczos:force_original_aspect_ratio=decrease,format=rgba,pad=2000:2000:(ow-iw)/2:(oh-ih)/2:color=#00000000,setsar=1').save('st.webp').on('end', async () => {
          await message.sendMessage(fs.readFileSync('st.webp'), MessageType.sticker);
        })
      })
...
Asena.addCommand({ pattern: 'ahttp ?(.*)', fromMe: wk, dontAddCommandList: true }, (async (message, match) => {
  if (message.reply_message) {
    ...
    await download(ttinullimage.image, '/root/Whats_Bot/ahttp.gif', async() => { 
      ffmpeg('/root/Whats_Bot/ahttp.gif').videoFilters('chromakey=black').save('af.gif').on('end', async () => {
        ffmpeg('/root/Whats_Bot/af.gif').outputOptions(["-y", "-vcodec libwebp", "-lossless 1", "-qscale 1", "-preset default", "-loop 0", "-an", "-vsync 0", "-s 600x600"]).videoFilters('scale=600:600:flags=lanczos:force_original_aspect_ratio=decrease,format=rgba,pad=600:600:(ow-iw)/2:(oh-ih)/2:color=#00000000,setsar=1').save('sticker.webp').on('end', async () => {
...
Asena.addCommand({ pattern: 'pttp ?(.*)', fromMe: wk, dontAddCommandList: true }, (async (message, match) => {
  if (message.reply_message) {
    ...
    await download(ttinullimage.image, '/root/Whats_Bot/pttp.gif', async() => { 
      ffmpeg('/root/Whats_Bot/pttp.gif').videoFilters('chromakey=white').save('af.gif').on('end', async () => {
        ffmpeg('/root/Whats_Bot/af.gif').outputOptions(["-y", "-vcodec libwebp", "-lossless 1", "-qscale 1", "-preset default", "-loop 0", "-an", "-vsync 0", "-s 600x600"]).videoFilters('scale=600:600:flags=lanczos:force_original_aspect_ratio=decrease,format=rgba,pad=600:600:(ow-iw)/2:(oh-ih)/2:color=#00000000,setsar=1').save('sticker.webp').on('end', async () => {
          await message.sendMessage(fs.readFileSync('sticker.webp'), MessageType.sticker);
        })
      })
