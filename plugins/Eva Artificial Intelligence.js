...
Eva, The WhatsBotAR's Multifunctional Artificial Intelligence
...
var vtalk_dsc = ''
var reply_eva = ''
if (conf.LANG == 'AR') vtalk_dsc = 'بدء إيفـا محادثتهـا الصوتيـة..', reply_eva = '*الـرد علـى أي بريـد صوتـي!*'
...
Asena.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
    if (message.message.startsWith('Eva') && eva_functionality !== 'true') {        
        ...
        var payload_client = await WhatsAsenaStack.get_eva_ai(encodeURIComponent(trmsg), 'Eva', 'WhatsBotAR', 'WhatsAsena', unique_ident)
        var fins = ''                           
        if (conf.LANG !== 'EN') {
...
Asena.addCommand({ pattern: 'تالك$', desc: vtalk_dsc, fromMe: wk }, (async (message, match) => {
    ...
    try {
        ...
        convertToWav(file)
            .on('end', async () => {
                ...
                var ainame = os.userInfo().homedir.split('Whats')[1].split('Duplicated/')[0]
                if (ainame !== 'Asena') return;
                var payload_client = await WhatsAsenaStack.get_eva_ai(encodeURIComponent(ssc), 'Eva', 'WhatsBotAR', 'WhatsAsena', unique_ident)
                var fins = ''                           
                if (conf.LANG !== 'EN') {
                    ceviri = await translatte(payload_client.result, {from: 'auto', to: conf.LANG});
                    if ('text' in ceviri) {
...
var wr_cmd = ''
if (conf.LANG == 'AR') {
    fulleva_dsc = 'ينشط ميزات Eva الوظيفية الكاملة. تحويل حسابك إلى روبوت محادثة بالذكاء الاصطناعي!'
    already_on = 'خاصية الذكاء الاصطناعي ..Eva.. تعمل بالفعل بشكل كامل.'
    already_off = 'خاصية الذكاء الاصطناعي ..Eva.. تعمل حاليًا بشكل شبه وظيفي.'
    succ_on = 'تـم تفعيل خاصية Eva وظيفيًا بالكامل! فضلا انتظر قليلا! ✅ '
    succ_off = 'تم تفعيل خاصية ..Eva.. بشكـل شبه وظيفي! فضلا انتظر قليلا! ☑️ '
    wr_cmd = 'الرجاء فقط استخدام الأمـر * تفعيل * أو * تعطيل *.'
}
if (conf.LANG == 'EN') {
...
Asena.addCommand({ pattern: 'ايفا ?(.*)', desc: fulleva_dsc, fromMe: true, usage: '.ايفا تفعيل / .ايفا تعطيل' }, (async (message, match) => {
