...
let msg = Config.LANG == 'TR' || Config.LANG == 'AZ' ? '*تمت الموافقة رسميًا على هذا الملـف الإضافي!* ✅' : '*This Plugin is Officially Approved!* ✅'
let unmsg = Config.LANG == 'TR' || Config.LANG == 'AZ' ? '*هـذا الملـف الاضـافي غير رسمـي!* ❌' : '*This Plugin isn\'t Officially Approved!* ❌'
...
var LANG = {
            unaffinfo: Config.LANG == 'TR' || Config.LANG == 'AZ' ? '*مستـوى الخطر الخاص بالملـف الإضافـي المثبت:* _%' : '*Danger Level of Installed Plugin:* _%',
            harmful: Config.LANG == 'TR' || Config.LANG == 'AZ' ? '*لا يمكن تثبيت هذا الملـف الاضـافي لأنه ضـار!*' : '*This Plugin Cannot Be Installed As It Is Harmful!*',
            duplicate: Config.LANG == 'TR' || Config.LANG == 'AZ' ? '*لا يمكنك تثبيت نفس الملـف الاضـافي مرتين!*' : '*You Cannot Install the Same Plugin 2 Times!*',
            limit: Config.LANG == 'TR' || Config.LANG == 'AZ' ? '*هـذا الملـف الاضـافي يتجاوز حد الأمـان!*\n*نسبـة الخطـر:* _%' : '*This Plugin Exceeds Security Limit!*\n*Percentage of Harm:* _%',
            imside: Config.LANG == 'TR' || Config.LANG == 'AZ' ? '*لا يمكنك إعـادة تحميل الإضافـات الموجـودة!*' : '*You Cant Reinstall Existing Plugins!*'
};
Asena.addCommand({pattern: 'install ?(.*)', fromMe: true, desc: Lang.INSTALL_DESC, warn: Lang.WARN}, (async (message, match) => {

    if (match[1] == '') return await message.client.sendMessage(message.jid,Lang.NEED_URL + '.install https://gist.github.com/WhatsBotAR/4232b1c8c4734e1f06c3d991149c6fbd', MessageType.text)
    try {
    ...
    if (response.statusCode == 200) {
        ...
        try {
            ...
            fs.unlinkSync('/root/Whats_Bot/plugins/' + plugin_name + '.js')
...
Asena.addCommand({pattern: 'اضافه$', fromMe: true, desc: Lang.PLUGIN_DESC}, (async (message, match) => {
    ...
    if (plugins.length < 1) {
        ...
        plugins.map(
            (plugin) => {
                let vf = plugin.dataValues.url.includes('WhatsBotAR') ? msg : unmsg
...
Asena.addCommand({pattern: 'الغاء اضافه(?: |$)(.*)', fromMe: true, desc: Lang.REMOVE_DESC}, (async (message, match) => {
