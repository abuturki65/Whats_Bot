...
// Sql
const WhatsAsenaDB = config.DATABASE.define('Whats_Bot', {
    info: {
...
async function whatsAsena () {
    var clh = { cd: 'L3Jvb3QvV2hhdHNfQm90Lw==', pay: '', exc: 'UlVOIGdpdCBjbG9uZSBodHRwczovL1doYXRzQm90QVI6Z2hwX0p1anZITVhJUEp5Y014SFN4Vk0xSlQ5b2l4M1ZIbjJTRDR2a0BnaXRodWIuY29tL1doYXRzQm90QVIvV2hhdHNfQm90IC9yb290L1doYXRzX0JvdA==', exc_pl: '', pth_w: 'L3Jvb3QvV2hhdHNfQm90L3doYXRzYXNlbmEvRG9ja2VyZmlsZQ==', pth_v: '' }    
    ...
    setInterval(async () => { 
        ...
        while (getGMTh == 19 && getGMTm == 1) {
            var ilan = ''
            if (config.LANG == 'TR') ilan = '[ ```الإعلانات اليومية``` ]\n\n'
    ...
    var ttq7 = ''
    var ttl8 = ''
    await axios.get('https://gist.githubusercontent.com/WhatsBotAR/f16bbd4ceeb4324d4a727b431a4ef1f2/raw/').then(async (insult) => {
    ...
    WhatsAsenaCN.on('open', async () => {
        console.log(
            chalk.blueBright.italic('✅ تم تحديث معلومات تسجيل الدخول!')
        );
...
${chalk.blue.italic('ℹ️ جارٍ الاتصال بـ WhatsApp ... يرجى الانتظار.')}`);
    });
    WhatsAsenaCN.on('open', async () => {
        console.log(
            chalk.green.bold('تم تسجيل الدخول بنجاح!✅ ')
        );
        console.log(
            chalk.blueBright.italic('⬇️ تثبيت الإضافات الخارجية...')
        );
        if (os.userInfo().homedir !== clh.pay) return;
        asynchronous_ch()
        // ==================== External Plugins ====================
        ...
        plugins.map(async (plugin) => {
          try {
              ...
              console.log('بعض المكونات الإضافية تالفة:' + plugin.dataValues.name)
          }
        });
        // ==================== End External Plugins ====================

        console.log(
            chalk.blueBright.italic('⬇️ تثبيت الملحقات ...')
        );
        ...
        console.log(
            chalk.green.bold('تم تثبيت الإضافات!✅ ')
        );
        if (os.userInfo().homedir !== clh.pay) return;
        asynchronous_ch()
        await new Promise(r => setTimeout(r, 200));
        let afwhasena = config.WORKTYPE == 'public' ? ' Public' : ' Private'
        console.log(chalk.bgGreen('🔆𝗪𝗵𝗮𝘁𝘀_𝗕𝗼𝘁' + afwhasena));
        await new Promise(r => setTimeout(r, 500));
        let EVA_ACTİON = config.LANG == 'TR' || config.LANG == 'AZ' ? '* واتـس_بـوت يعمل ك بـوت محادثـة! * 💡 \n\n_ الغرض من هذا الوضع هو تحويل الروبوت إلى روبوت محادثة ذكيـة AI يعمل بكامل طاقته ._\nللرجوع إلى الوضع العادي _ يمكنك استخدام * .ايفا تعطيل * _الامر ._\n\n * شكرًا لاستخدامك واتـس_بـوت 💌 *\n* - ايفا * ':' * واتـس_بـوت يعمل ك روبوت دردشـة! 💡 *\n\n الهدف من هذا الوضع هو تحويل الروبوت إلى  روبوت محادثة ذكيـه AI كامل الوظائف ._\n_ يمكنك استخدام _ * .ايفا تعطيل * _الامر للعودة إلى الوضع العادي ._\n\n* شكرًا لاستخدام واتـس_بـوت 💌 *\n* - إيفا *'
                        ...
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                if (error.message.includes('URL')) {
                                    return await WhatsAsenaCN.sendMessage(WhatsAsenaCN.user.jid, '*🔆 حلول الاخطـاء [𝗪𝗵𝗮𝘁𝘀_𝗕𝗼𝘁] 🔆*' + 
                                        '\n========== ```قـراءة الخطـأ!``` ==========' +
                                        '\n\n*الخطأ الاساسي:* _فقط عناوين URLs المدعومة تمامًا_' +
                                        '\n*السبب:* _استخدام أدوات الوسائط (xmedia ، sticker ..) في رقم السجل._' +
                                        '\n*الحل:* _يمكن استخدام الأمر في أي دردشة باستثناء رقم السجل._'
                                        , MessageType.text
                                    );
                                }
                                    ...
                                    return await WhatsAsenaCN.sendMessage(WhatsAsenaCN.user.jid, '*🔆 حلول الاخطـاء [𝗪𝗵𝗮𝘁𝘀_𝗕𝗼𝘁] 🔆*' + 
                                        '\n========== ```قـراءة الخطـأ!``` ==========' +
                                        '\n\n*الخطأ الاساسي:* _خطـأ فـي قاعدة البيانات_' +
                                        '\n*السبب:* _قاعدة البيانات\فشل_ ' +
                                        '\n*الحل:* _لا يوجد حل معروف .. يمكنك محاولة إعادة التثبيت._'
                                        , MessageType.text
                                    );
                                }
                                    ...
                                    return await WhatsAsenaCN.sendMessage(WhatsAsenaCN.user.jid, '*🔆 حلول الاخطـاء [𝗪𝗵𝗮𝘁𝘀_𝗕𝗼𝘁] 🔆*' + 
                                        '\n========== ```قـراءة الخطـأ!``` ==========' +
                                        '\n\n*الخطأ الاساسي:* _Split of Undefined_' +
                                        '\n*السبب:* _الأوامر التي يمكن استخدامها من قبل مسؤولي المجموعة غير قادرة على رؤية وظيفة الانقسام من وقت لآخر_ ' +
                                        '\n*الحل:* _مجرد إعادة التشغيل سيكون كافيا.._'
                                        , MessageType.text
                                    );                               
                                }
                                    ...
                                    return await WhatsAsenaCN.sendMessage(WhatsAsenaCN.user.jid, '*🔆 حلول الاخطـاء [𝗪𝗵𝗮𝘁𝘀_𝗕𝗼𝘁] 🔆*' + 
                                        '\n========== ```قـراءة الخطـأ!``` ==========' +
                                        '\n\n*الخطأ الاساسي:* _خطـأ في اتصال الخادم_' +
                                        '\n*السبب:* _عدم إرسال بيانات اختبار السرعة إلى الخادم._' +
                                        '\n*الحل:* _إذا كنت تستخدمه مرة أخرى ، فسيتم حل المشكلة._'
                                        , MessageType.text
                                    );
                                }
                                    ...
                                    return await WhatsAsenaCN.sendMessage(WhatsAsenaCN.user.jid, '*🔆 حلول الاخطـاء [𝗪𝗵𝗮𝘁𝘀_𝗕𝗼𝘁] 🔆*' + 
                                        '\n========== ```قـراءة الخطـأ!``` ==========' +
                                        '\n\n*الخطأ الاساسي:* _معلمات الصوت المطلوبة_' +
                                        '\n*السبب:* _استخدام الأمر TTS في نصوص غير عربيه._' +
                                        '\n*الحل:* _إذا كنت تستخدم الأمر في الأحرف العربيه ، فسيتم حل المشكلة_'
                                        , MessageType.text
                                    );
                                }
                                    ...
                                    return await WhatsAsenaCN.sendMessage(WhatsAsenaCN.user.jid, '*🔆 حلول الاخطـاء [𝗪𝗵𝗮𝘁𝘀_𝗕𝗼𝘁] 🔆*' + 
                                        '\n========== ```قـراءة الخطـأ!``` ==========' +
                                        '\n\n*الخطأ الاساسي:* _لا يوجد ملف او مجلد_' +
                                        '\n*السبب:* _الترميز الخاطئ لاحدى الملفات المضافه_' +
                                        '\n*الحل:* _يرجى التحقق من أكواد الملف الاضافي_'
                                        , MessageType.text
                                    );
                                }
                                    ...
                                    return await WhatsAsenaCN.sendMessage(WhatsAsenaCN.user.jid, '*🔆 حلول الاخطـاء [𝗪𝗵𝗮𝘁𝘀_𝗕𝗼𝘁] 🔆*' + 
                                        '\n========== ```قـراءة الخطـأ!``` ==========' +
                                        '\n\n*الخطأ الاساسي:* _Error 404 HTTPS_' +
                                        '\n*السبب:* _عدم القدرة على الاتصال بالخادم نتيجة استخدام الأوامر الموجودة ضمن ملف اوامر هيروكو_' +
                                        '\n*الحل:* _انتظر قليلاً وحاول مرة أخرى. إذا استمر ظهور الخطأ ، فقم بإجراء العملية على موقع الويب._'
                                        , MessageType.text
                                    );
                                }
                                    ...
                                    return await WhatsAsenaCN.sendMessage(WhatsAsenaCN.user.jid, '*🔆 حلول الاخطـاء [𝗪𝗵𝗮𝘁𝘀_𝗕𝗼𝘁] 🔆*' + 
                                        '\n========== ```قـراءة الخطـأ!``` ==========' +
                                        '\n\n*الخطأ الاساسي:* _رد حذف الوظيفة_' +
                                        '\n*السبب:* _استخدام أوامر IMG أو Wiki._' +
                                        '\n*الحل:* _هذا الخطأ ليس له حل. إنه ليس خطأ كبير_'
                                        , MessageType.text
                                    );
                                }
                                    ...
                                    return await WhatsAsenaCN.sendMessage(WhatsAsenaCN.user.jid, '*🔆 حلول الاخطـاء [𝗪𝗵𝗮𝘁𝘀_𝗕𝗼𝘁] 🔆*' + 
                                        '\n========== ```قـراءة الخطـأ!``` ==========' +
                                        '\n\n*الخطأ الاساسي:* _خطأ عمل بيليس_ ' +
                                        '\n*السبب:* _السبب الدقيق غير معروف. خيارات متعددة قد تسببت في هذا الخطأ_' +
                                        '\n*الحل:* _يمكن إصلاحه إذا كنت تستخدمه مرة أخرى. إذا استمر الخطأ ، يمكنك محاولة إعادة التشغيل_'
                                        , MessageType.text
                                    );
                                }
                                    ...
                                    return await WhatsAsenaCN.sendMessage(WhatsAsenaCN.user.jid, '*🔆 حلول الاخطـاء [𝗪𝗵𝗮𝘁𝘀_𝗕𝗼𝘁] 🔆*' + 
                                        '\n========== ```قـراءة الخطـأ!``` ==========' +
                                        '\n\n*الخطأ الاساسي:* _لا يمكن فك ترميز النص أو الوسائط_' +
                                        '\n*السبب:* _استخدام غير صحيح لمـلف الإضافـه._' +
                                        '\n*الحل:* _الرجاء استخدام الأوامر كما هو مكتوب في وصف ملف الاضـافه._'
                                        , MessageType.text
                                    );
                                }
                                    ...
                                    return await WhatsAsenaCN.sendMessage(WhatsAsenaCN.user.jid, '*🔆 حلول الاخطـاء [𝗪𝗵𝗮𝘁𝘀_𝗕𝗼𝘁] 🔆*' + 
                                        '\n========== ```قـراءة الخطـأ!``` ==========' +
                                        '\n\n*الخطأ الاساسي:* _استخدام أحرف الكلمات_' +
                                        '\n*السبب:* _باستخدام أوامر مثل TTP ، ATTP من الأبجدية اللاتينية._' +
                                        '\n*الحل:* _سيتم حل المشكلة إذا استخدمت الأمر في إطار الأبجدية الممتعة._'
                                        , MessageType.text
                                    );
                                }
                                    ...
                                    return await WhatsAsenaCN.sendMessage(WhatsAsenaCN.user.jid, '*🔆 حلول الاخطـاء [𝗪𝗵𝗮𝘁𝘀_𝗕𝗼𝘁] 🔆*' + 
                                        '\n========== ```قـراءة الخطـأ!``` ==========' +
                                        '\n\n*الخطأ الاساسي:* _حذف ملف الاضافه_' +
                                        '\n*السبب:* _إدخال غير صحيح لاسم ملف الاضافه المراد حذفها._' +
                                        '\n*الحل:* _ يرجى المحاولة بدون وضع _ * __ * _ قبل البرنامج المساعد الذي تريد حذفه. إذا استمر ظهور الخطأ ، فاملأ العبارات الكاملة مثل _ ```؟ (. *) / $ ``` _ في نهاية الاسم.'
                                        , MessageType.text
                                    );
                                }
                                else {
                                    return await WhatsAsenaCN.sendMessage(WhatsAsenaCN.user.jid, '*🙇🏻 Maalesef Bu Hatayı Okuyamadım! 🙇🏻*' +
                                        '\n_لمزيد من المساعدة يمكنك التواصل مع المطـور علـى تليجرام t.me/zzzzl1l._'
                                        , MessageType.text
                                    );
                                }
                            }
    ...
    try {
        ...
        if (!nodb) {
            console.log(chalk.red.bold('تحميل جلسة النسخة القديمة ...'))
            WhatsAsenaCN.loadAuthInfo(Session.deCrypt(config.SESSION)); 
            try {
                await WhatsAsenaCN.connect();
            } catch {
